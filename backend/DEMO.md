...
### A coherent catalog for demonstrations

The eight-product smoke catalog above is a set of *fixtures* — each item proves a specific behaviour, and the smoke suite asserts relationships against it. It is deliberately scattered (footwear, kitchenware, bags, lighting) and priced 24–135.

That makes it a poor **demonstration**. With one or two same-category comparables, every price estimate honestly reports `low` confidence; and a real upload priced in the thousands against a catalog topping out at 135 cannot produce a sensible number. The output ends up describing the seed data rather than the system.

For demos, seed a coherent catalog instead:

```bash
python scripts/seed_catalog.py            # 24 products, one category, one price scale
python scripts/seed_catalog.py --list     # show it without contacting a deployment
```

Twenty-four running and training shoes across six brands and four segments (road, trail, court, lifestyle), priced 2,499–14,999 on a single scale. Images are generated by the same deterministic, dependency-free drawing code as the smoke dataset — nothing is downloaded, and every product is named `Demo …` and described as synthetic.

Measured difference on the same product (listed 8,499):

| Catalog | Estimate | Confidence | Comparables |
|---|---|---|---|
| Smoke fixtures (mixed categories) | 127.49 | `low` (0.31) | 2 |
| Coherent catalog | **7,386.50** | **`medium`** (0.52) | **20** |

> [!IMPORTANT]
> Seed this into a **clean index**. The two catalogs share the `footwear` category, so running both together drags estimates across incompatible price scales — measured at 5,657 for the same product, with 129.99 shoes sitting beside 8,999 ones. Reset first with `make reset` (or drop just the data volumes and keep `model_cache` to avoid re-downloading models).
