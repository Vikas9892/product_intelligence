...
overrides the `get_upload_service` dependency
(`app.dependency_overrides[get_upload_service] = ...`) to redirect storage
to `tmp_path` — this is exactly the seam `app/dependencies/` exists for
(see that package's module docstring), and means these tests exercise the
...
class TestUploadProductSuccess:
    def test_returns_201_with_product_and_image_metadata(
